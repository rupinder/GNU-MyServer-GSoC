...
The linux portion is not suitable for running as root due to posable secuity flaws in the program.  To run the server as a user, you must change the port to something like 8080 in the myserver.xml file because users can only use port numbers larger than 1024. 
